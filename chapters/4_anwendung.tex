...
In der durchgeführten Fallstudie zeigen die Autoren, dass alle mit ihrem Ansatz produzierten Microservices-Kandidaten bessere Ergebnisse liefern als bei der Durchführung mit \emph{Random Search}.
...
\result{4} ermöglicht die automatische Umgestaltung eines Java-Monolithen in Microservices unter Angabe einer Dekomposition.
...
Während die Eingabe, die die Verfahren voraussetzen sehr unterschiedlich sein kann, ist die Funktionsweise, wie mit den eingegebenen Daten fortgefahren wird, oft sehr ähnlich.
...
Die Gruppierung wird anhand bestimmter Metriken optimiert, wobei Kohäsion und Coupling die häufigsten sind.
...
Der erste und in diesem Fall entscheidendste Aspekt, in dem sich die Methoden unterschieden, ist die Art der Eingabe.
...
Nachdem durch die Eingabe ein Ausgangsgraph erstellt wurde, besteht der nächste Unterschied der Methoden darin, wie aus diesem hochqualitative Microservices identifiziert werden können.
...
Abgesehen von diesen beiden Hauptkategorien der Unterschiede in der Funktionsweise gibt es noch viele weitere Unterschiede in den Verfahren.
...
Doch entweder konnte das Werkzeug nicht öffentlich gefunden werden (Ergebnisse \resultref{2} und \resultref{3}), das Werkzeug ist nicht mit den Technologien von \jf kompatibel (\resultref{5}) oder es existiert kein Werkzeug (Ergebnisse \resultref{7} und \resultref{8}).

Im Vergleich ist \result{3} ein Favorit.
...
Diese beiden Verfahren werden als Favoriten für die weitere Planung im nächsten Abschnitt verwendet.


%Wie auch in der Übersicht über die restlichen Verfahren in \cref{tab:phase2-comparison} werden diese und die Anwendbarkeit dieser auf \jf im Folgenden nach Bewertung des Autors und \gls{po} geordnet (\cref{feldnotiz:7,feldnotiz:11,feldnotiz:14}) diskutiert.
%
%\input{tables/phase2-method-comparison}
%
...
Im vorherigen Abschnitt wurden die verschiedenen Verfahren nach Eignung geordnet.
In diesem Abschnitt wird die Planung des weiteren Vorgehens beschrieben.
%In dieser Reihenfolge werden sie in diesem Abschnitt detaillierter betrachtet.
Dabei ist zu beachten, dass das Ziel in dieser Arbeit nicht ist, einen der Ansätze originalgetreu umzusetzen und diesen spezifisch zu testen, sondern ein geeignetes Vorgehen für den Spezialfall zu finden.
Dieses kann aus der Abwandlung eines Verfahrens entstehen, aber auch durch die Kombination mehrerer Verfahren oder die zusätzliche Verwendung eines Tools.
Wegen des zeitlich begrenzten Rahmens des Refactoring-Prozess in dieser Thesis ist eines der wichtigsten Kriterien für die Planung des \gls{sia} der zeitliche Aufwand der Methode.

Begonnen wurde mit der Suche nach dem Tool \gls{mb} des Ergebnisses \resultref{3}.
Dieses konnte nicht in dem Artikel oder auf anderen offiziellen Wegen gefunden werden.
...
Dieses konnte allerdings auch nach der Behebung mehrerer Fehler beim Starten nicht verwendet werden, da bei jeder Navigation von der Startseite des Web-basierten Projekts Datenbank-Fehler auftraten.
Da keine Dokumentation vorhanden ist, der Code in Teilen auf Brasilianisch geschrieben ist und die Autoren nicht auf Nachfrage per E-Mail reagierten, wird die Verwendung von \gls{mb} ausgeschlossen.
Wie bereits im vorherigen Abschnitt erwähnt, war \gls{mb} zwar der Favorit, wurde allerdings ohne funktionierendes Werkzeug als zu aufwendig eingeschätzt.

Die nächstbeste Option ist dadurch die Verwendung von \result{2}.
