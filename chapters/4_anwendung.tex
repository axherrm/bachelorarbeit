...
Im vorherigen Abschnitt wurden die verschiedenen Verfahren nach ihrer Eignung geordnet.
...
Da keine Dokumentation vorliegt, der Code teilweise in brasilianischer Sprache geschrieben ist und die Autoren auf Anfrage per E-Mail nicht reagierten, scheidet die Verwendung von \gls{mb} aus.
Wie bereits im vorherigen Abschnitt erwähnt, war \gls{mb} zwar der Favorit, wurde aber ohne funktionierendes Werkzeug als zu aufwendig erachtet.
...
Auch eine Anfrage per E-Mail brachte keine Antwort.
...
Als hauptsächliches Forschungsergebnis dieses Verfahrens wurde im Aritkel der entworfene Algorithmus \emph{cHAC} genannt, weshalb mit dessen Umsetzung begonnen wurde.
Es wurde versucht, diesen mit Top-Down Vorgehen in Java zu implementieren.
...
An einigen Stellen wurden die erwähnten Schritte des Algorithmus nicht definiert, sondern nur oberflächlich das Ziel der Methode beschrieben und die Umsetzung offengelassen.
An anderen Stellen wurden im Algorithmus erwähnte Methoden im Text durch Formeln definiert, wobei teilweise unklar war, ob diese Formeln korrekt sind.
Es werden einige Unklarheiten wie Schreib- und Logikfehler im Algorithmus und in einer Formel vermutet, die mit der relativ dürftigen Beschreibung im Artikel nicht geklärt werden konnten.
...
Da die anderen Suchergebnisse des \gls{arh} bereits in den vorangegangenen Schritten als deutlich weniger geeignet bewertet wurden, wird aus diesem Grund und aus Zeitgründen keine weitere Anwendung angestrebt.

\subsection{Suche nach Best Practices und Patterns}

Die Suche nach \bpp wird wie auch die Suche nach Migrationsverfahren lediglich durch die definierten \glspl{qa} beeinflusst.
